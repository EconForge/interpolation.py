...
#  cost of using Ad, dAd as frozen variables ?
...
@njit
def kernel(i, a, b, orders, coefs, points, values):
...
if __name__ == '__main__':
    ...
    coeffs = np.random.random(orders)
    ...
    pp = points.T.copy()
    # pp = points
    vec_eval_cubic_spline_3_inlined(a,b,orders,coeffs,pp,vals)
    t3 = time.time()
    vec_eval_cubic_spline_3_kernel(a,b,orders,coeffs,points,vals)
    t4 = time.time()
    print("one function call per point: {}".format(t2-t1))
    print("inlined: {}".format(t3-t2))
    print("kernel: {}".format(t4-t3))
    print(vals[:10,0])


    # ll = vec_eval_cubic_spline_3_inlined.inspect_llvm()
    # k = next( iter(ll.keys()))
    # print( ll[k] )
