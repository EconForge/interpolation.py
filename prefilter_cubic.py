...
D = 1+np.random.random((K,)*d)
C = np.random.random((K+2,)*d)
CC = np.random.random((K+2,)*d+(5,))
DD = 1+np.random.random((K,)*d+(5,))

grid
D.shape
C.shape

%timeit prefilter_cubic(grid, D, C)

%timeit filter_coeffs(a,b,n,D)




prefilter_cubic(grid, D)

prefilter_cubic(grid, DD)



from interpolation.splines.filter_cubic import filter_coeffs, filter_mcoeffs

cc = filter_coeffs(a,b,n,D)
...
%timeit prefilter_cubic(grid, D)
%timeit filter_coeffs(a,b,n,D)



prefilter_cubic(grid, DD, CC)
cc = filter_mcoeffs(a,b,n, DD)

abs(CC - cc).max()


%timeit prefilter_cubic(grid, DD)
%timeit cc = filter_mcoeffs(a,b,n, DD)
